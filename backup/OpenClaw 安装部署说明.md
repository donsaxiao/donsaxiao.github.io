...
# 五、飞书渠道配置
截止目前，整个OpenClaw已经完全部署完成，我们接着在配置下飞书渠道。配置飞书渠道我们首先需要在飞书中创建一个可用的机器人，参照如下步骤逐步完成即可。 说明：本次介绍使用的是飞书个人账号，如果是企业账号则版本发布以及权限授权需要进行管理员审批，其他操作步骤一致。 
...
## 步骤 5.4：
获取凭证并1Panel配置 进入飞书平台在「凭证与基础信息」中获取应用凭证，如下图所示：
